...
MINS=`gdalinfo $RASTER | grep "Lower Left" | sed 's/Lower Left  //g' | sed 's/(//g' | sed 's/)//g' | sed 's/,//g' | tr -s ' ' | sed -e 's/^[[:space:]]*//' | cut -d' ' -f "1 2"`
MAXS=`gdalinfo $RASTER | grep "Upper Right" | sed 's/Upper Right //g' | sed 's/(//g' | sed 's/)//g' | sed 's/,//g' | tr -s ' ' | sed -e 's/^[[:space:]]*//' | cut -d' ' -f "1 2"`
